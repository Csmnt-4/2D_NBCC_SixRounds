...
std::vector<std::tuple<glm::vec2, Sprite*>> hugeBananas, duplicate;
std::vector<Sprite*> pressStart;
std::map<char, Sprite*> alphabetAndOtherCharacters;

int currentFrame = 0;
double elapsedTime = 0;
double elapsedTimeAnimation = 0;
...
void Init()
{
	...
	//	alphabetAndOtherCharacters[letter] = blit3D->MakeSprite(64, 0, 64, 64, "Media\\Alphabet\\" + std::to_string(letter) + ".png");
	//}

	//for (int number = 0; number < 10; number++) {
	//	alphabetAndOtherCharacters[static_cast<char>(number)] = blit3D->MakeSprite(64, 0, 64, 64, "Media\\Numbers\\" + std::to_string(number) + ".png");
	//}
	
	while (hugeBananas.size() <= 1024) {
		for (int i = 0; i <= dice.Roll1DN(5); ++i) {
			Sprite* banana;
			if (dice.Roll1DN(2) == 1)
				banana = blit3D->MakeSprite(0, 0, 160, 160, "Media\\huge_banana.png");
			else
				banana = blit3D->MakeSprite(0, 0, 160, 160, "Media\\huge_bullet.png");
			...
			duplicate.push_back(std::tuple<glm::vec2, Sprite*>{glm::vec2(dice.Roll1DN(1920), dice.Roll1DN(1080)), banana});
		}
	}
...
void Update(double seconds)
{
	...
	while (elapsedTime >= timeSlice)
	{
		...
		if (elapsedTimeAnimation >= 0.1) {
			if (currentFrame == 11)
				currentFrame = 0;
			else currentFrame++;
			elapsedTimeAnimation -= 0.1;
		}

		if (hugeBananas.size() >= 1000 && (int)gameState < 1) {
			if (!duplicate.empty()) {
				if (duplicate.size()>500)
					duplicate.pop_back();
				if (duplicate.size() > 300)
					duplicate.pop_back();
				if (!duplicate.empty())
					duplicate.erase(duplicate.begin() + dice.Roll1DN(duplicate.size()) - 1);
				if (!duplicate.empty())
					duplicate.erase(duplicate.begin() + dice.Roll1DN(duplicate.size()) - 1);
			}
		}
		...
		if ((int)gameState == 1) {
			timeSlice = 1.f / 120.f;
			if ((int)gameState < 2 && !hugeBananas.empty()) {
			...
			if (drid.player.healthPoints < 1) {
				drid.appendToFile("Scores.txt", drid.score);
				drid.loadTextFile("Scores.txt");
...
void Draw(void)
{
	...
	if (!hugeBananas.empty()) {
		...
		if (hugeBananas.size() >= 800 && (int)gameState < 1)
			pressStart[currentFrame]->Blit(1920.f / 2, 1080.f / 2);

			for (auto& tuple : duplicate)
				std::get<1>(tuple)->Blit(std::get<0>(tuple).x, std::get<0>(tuple).y);
		}

	if (!drid.bufferString.empty()) {
	}
